...
async function seedAll16() {
  try {
    ...
    for (const item of all16DefaultUnis) {
      const existing = await University.findOne({ slug: item.slug });
      if (!existing) {
        await University.create({
          ...item,
          country: defaultCountry._id,
        });
        console.log(`Seeded new university to DB: ${item.name}`);
      } else {
        // Fix cover image if tattoo or empty
        if (!existing.coverImage || existing.coverImage.includes('1568515045052')) {
          existing.coverImage = item.coverImage;
          await existing.save();
          console.log(`Updated cover image in DB for: ${existing.name}`);
        }
      }
    }

    const count = await University.countDocuments();
    console.log(`Total universities in MongoDB now: ${count}`);
    process.exit(0);
  } catch (err) {
    console.error('Error seeding 16 universities:', err);
    process.exit(1);
  }
}
