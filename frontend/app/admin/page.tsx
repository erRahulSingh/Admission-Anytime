  ...
  return (
    <div className="space-y-5" style={{ fontFamily: "'Inter', 'Plus Jakarta Sans', system-ui, sans-serif" }}>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-3 sm:gap-4">
        {kpis.map((k, i) => (
          <div key={i} className="bg-white rounded-xl border border-[#eef0f4] p-3 sm:p-4 flex flex-col justify-between hover:shadow-md transition-shadow">
            <div className="flex items-start justify-between mb-2 sm:mb-3">
              <span className="text-[9px] sm:text-[10px] font-semibold text-[#8c95a6] uppercase tracking-wider leading-tight">{k.label}</span>
              <div className={`w-7 h-7 sm:w-8 sm:h-8 rounded-full ${k.iconBg} flex items-center justify-center text-[12px] sm:text-[13px] flex-shrink-0`}>
                {k.icon}
              </div>
            </div>
            <div>
              <h3 className="text-[18px] sm:text-[22px] font-extrabold text-[#1a1f36] leading-none tracking-tight">{fmt(k.value)}</h3>
              <div className="flex items-center gap-1 mt-1 sm:mt-1.5">
                <span className="text-[9px] sm:text-[10px] font-bold text-emerald-500 flex items-center gap-0.5">
                  <FaArrowUp className="text-[7px]" /> {k.pctUp}%
                </span>
                <span className="text-[9px] sm:text-[10px] text-[#a0aab8]">vs Apr 01 - Apr 30</span>
              </div>
            </div>
          </div>
        ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        {/* Leads Overview */}
        <div className="lg:col-span-4 bg-white rounded-xl border border-[#eef0f4] p-4 sm:p-5">
              ...
              {overviewSegs.map((seg, i) => (
                <div key={i} className="flex items-center justify-between text-[11px]">
                  <div className="flex items-center gap-1.5 min-w-0">
                    ...
                    <span className="text-[#5a6478] font-medium truncate">{seg.label}</span>
                  </div>
                  <span className="font-bold text-[#1a1f36] flex-shrink-0 ml-2">
        ...
        {/* Leads Trend */}
        <div className="lg:col-span-5 bg-white rounded-xl border border-[#eef0f4] p-4 sm:p-5 overflow-hidden">
          <div className="flex justify-between items-center mb-1">
            <h4 className="text-[13px] font-bold text-[#1a1f36]">Leads Trend</h4>
            <button className="text-[11px] text-[#8c95a6] font-medium flex items-center gap-1">This Month <FaChevronDown className="text-[8px]" /></button>
          </div>
          {/* Legend */}
          <div className="flex flex-wrap items-center gap-3 sm:gap-4 mb-3 text-[10px] text-[#8c95a6] font-medium">
          ...
          <div className="w-full overflow-x-auto">
            <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto min-w-[280px]">
              <defs>
        ...
        {/* Leads Source */}
        <div className="lg:col-span-3 bg-white rounded-xl border border-[#eef0f4] p-4 sm:p-5">
          <div className="flex justify-between items-center mb-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        ...
        <div className="lg:col-span-4 bg-white rounded-xl border border-[#eef0f4] p-4 sm:p-5">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4">
            {/* Funnel SVG */}
            <svg viewBox="0 0 160 180" className="w-[130px] sm:w-[140px] flex-shrink-0">
            ...
            {/* Labels */}
            <div className="space-y-2.5 sm:space-y-3 flex-1 w-full min-w-0">
              {funnelLayers.map((layer, i) => (
                <div key={i} className="flex justify-between sm:block">
                  <div className="text-[14px] sm:text-[16px] font-extrabold text-[#1a1f36] leading-none">{fmt(layer.value)}</div>
        ...
        <div className="lg:col-span-4 bg-white rounded-xl border border-[#eef0f4] p-4 sm:p-5">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-5">
            ...
            <div className="space-y-2.5 sm:space-y-3 flex-1 w-full min-w-0">
              {admSegs.map((seg, i) => (
                <div key={i} className="flex items-center justify-between text-[11px] sm:text-[12px]">
            ...
            {d.topCourses.map((c, i) => (
              <div key={i}>
                <div className="flex justify-between text-[12px] mb-1">
                  <span className="text-[#3d4555] font-medium truncate pr-2">{c.course}</span>
                  <span className="font-bold text-[#1a1f36] flex-shrink-0">{c.percentage}%</span>
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        ...
        <div className="lg:col-span-5 bg-white rounded-xl border border-[#eef0f4] p-4 sm:p-5">
          <div className="flex justify-between items-center mb-4">
              ...
              {d.locations.map((loc, i) => (
                <div key={i}>
                  <div className="flex justify-between text-[11px] mb-1">
                    <span className="text-[#3d4555] font-semibold truncate pr-2">{loc.state}</span>
                    <span className="text-[#8c95a6] font-medium flex-shrink-0">{fmt(loc.value)} ({loc.percentage}%)</span>
                  </div>
        ...
        {/* Recent Leads */}
        <div className="lg:col-span-7 bg-white rounded-xl border border-[#eef0f4] p-4 sm:p-5 overflow-hidden">
          <div className="flex justify-between items-center mb-4">
